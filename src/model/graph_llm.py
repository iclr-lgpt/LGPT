...
class GraphLLM(torch.nn.Module):
    ...
    def encode_graphs(self, samples):
        ...
        # Processing a graph with zero nodes
        num_graph = len(graphs['ptr'])-1
        prev = graphs['ptr'][0]
        zero_node_list=[]
        for i, ptr in enumerate(graphs['ptr'][1:]):
            if ptr == prev:
        ...
        if len(zero_node_list) != 0 :
            for i in range(num_graph):
                if i in zero_node_list:
                    embedding_list.append(torch.zeros_like(g_embeds[0]))
                else:
                    embedding_list.append(g_embeds[cur])
                    cur += 1
            g_embeds = torch.stack(embedding_list)
        else:
            g_embeds = g_embeds
