...
class CrossQAGLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.graph_encoder = load_gnn_model['cross_qag'](
        ...
        self.projector = nn.Sequential(
            nn.Linear(args.gnn_hidden_dim, self.model.config.hidden_size, dtype=torch.bfloat16),
            nn.ReLU(),
    ...
    def encode_graphs(self, samples, pooling='graph_token'):
        graphs = samples['graph']
        samples['graph'] = graphs.to(self.model.device)
        g_embeds, _ = self.graph_encoder(samples, pooling)
